...
import { Quest, QuestBook, QuestLines as QuestLine } from "./transformQuestBookTypes";
import { cloneDirectory } from "../../globals";
...
export async function transformQuestBook(): Promise<void> {
	log("Transforming Quest Book...");
	const sharedQBDefaults = upath.join(cloneDirectory, "overrides", "config", "betterquesting");
	const sharedConfigOverrides = upath.join(cloneDirectory, "overrides", "config-overrides");
	...
	// Quest Lang Location
	const questLangLocation = upath.join(cloneDirectory, "overrides", langFileLocation);
	...
	await fs.promises.writeFile(questPathExpertOverride, JSON.stringify(questBookExpert, null, 2));

	log("Transformed Quest Book!");
}
